...
//FIND HISTORICAL LIFE EXPECTANCY

interface CenturyGroup {century: number;ages: Array<number>;};
  
function lifeExpectancy(array: Ancestor[]):any {

    var centuries: Array<CenturyGroup> = [];
    var ret: Array<any> = [];
    var ret: Array<any> = [];
    ...
    array.forEach(function(n){
        ...
        var year: number = Math.ceil(n.died/100);
        var age: number = n.died - n.born;
        
        //Keep track of unique years     
        if(years.indexOf(year)== -1) {
            years.push(year);
            centuries.push({century: year,ages: []})
        }
        centuries[year].ages.push(age);
    })

    centuries.forEach(function(a){
        ret.push(a.century + ' ' + average(a.ages));
    })
