...
["my car", "bad cats"], ["camper", "high art"]);
verify(new RegExp('pr?op'), //pop and prop
["pop culture", "mad props"], ["plop"]);
verify(new RegExp('(ferr)et|y|ari'), //ferret ferry and ferrari
["ferret", "ferry", "ferrari"], ["ferrum", "transfer A"]);
verify(new RegExp('ious\\b'), ["how delicious", "spacious room"], // any word ending in ious
["ruinous", "consciousness"]);
verify(new RegExp('.*\\s[.:;,]'), // a whitespace character followed by a dot, comma, colon or semicolon 
["bad punctuation ."], ["escape the dot"]);
verify(new RegExp('\\w{7,}'), // a word longer than 6 letters 
["hottentottententen"], ["no", "hotten totten tenten"]);
verify(new RegExp('\\b[a-df-z]+\\b'), // a word without the letter e 
["red platypus", "wobbling nest"], ["earth bed", "learning ape"]);
...
// Change this call.
console.log(text.replace(new RegExp('\'(?=[A-Z])|(\'\\W|\\W\')|\'$', 'g'), (str) => {
    return str.replace("'", '"');
}));
...
var number = new RegExp('[+-]?\\d?[\\.]?\\d?[Ee]?[+-]?\\d*');
// Tests:
